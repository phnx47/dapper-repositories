...
namespace MicroOrm.Dapper.Repositories.Tests.SqlGeneratorTests
{
    public class MsSqlGeneratorTests
    {
        ...
        public static void UpdateExclude()
        {
            ...
            Assert.Equal("UPDATE [DAB].[Phones] SET [DAB].[Phones].[Number] = @PhoneNumber, [DAB].[Phones].[IsActive] = @PhoneIsActive WHERE [DAB].[Phones].[Id] = @PhoneId",
                sqlQuery.GetSql());
        }
        ...
        public static void SelectGroupConditionsWithPredicate()
        {
            ...
            Assert.Equal(
                sPrefix + "([DAB].[Phones].[IsActive] = @IsActive_p0 AND [DAB].[Phones].[Id] = @Id_p1) OR ([DAB].[Phones].[Id] = @Id_p2 AND [DAB].[Phones].[Number] = @Number_p3)",
                sqlQuery1.GetSql());
            ...
            Assert.Equal(
                sPrefix +
                "[DAB].[Phones].[Number] = @Number_p0 AND ([DAB].[Phones].[IsActive] = @IsActive_p1 OR [DAB].[Phones].[Number] = @Number_p2) AND [DAB].[Phones].[Id] = @Id_p3",
                sqlQuery4.GetSql());

            var sqlQuery5 = phoneSqlGenerator.GetSelectAll(x => x.Number == "1" && (x.IsActive || x.Number == "456" || x.Number == "678") && x.Id == 123, null);
            Assert.Equal(
                sPrefix +
                "[DAB].[Phones].[Number] = @Number_p0 AND ([DAB].[Phones].[IsActive] = @IsActive_p1 OR [DAB].[Phones].[Number] = @Number_p2 OR [DAB].[Phones].[Number] = @Number_p3) AND [DAB].[Phones].[Id] = @Id_p4",
                sqlQuery5.GetSql());
            ...
            Assert.Equal(
                sPrefix + "[DAB].[Phones].[IsActive] = @IsActive_p0 AND [DAB].[Phones].[Id] = @Id_p1 AND [DAB].[Phones].[Id] = @Id_p2 AND [DAB].[Phones].[Number] = @Number_p3",
                sqlQuery7.GetSql());

            var sqlQuery8 = phoneSqlGenerator.GetSelectAll(
                x => x.Number == "1" && (x.IsActive || x.Number == "456" || x.Number == "123" || (x.Id == 1213 && x.Number == "678")) && x.Id == 123, null);
            Assert.Equal(
                sPrefix +
                "[DAB].[Phones].[Number] = @Number_p0 AND ([DAB].[Phones].[IsActive] = @IsActive_p1 OR [DAB].[Phones].[Number] = @Number_p2 OR [DAB].[Phones].[Number] = @Number_p3 OR ([DAB].[Phones].[Id] = @Id_p4 AND [DAB].[Phones].[Number] = @Number_p5)) AND [DAB].[Phones].[Id] = @Id_p6",
                sqlQuery8.GetSql());

            var sqlQuery9 = phoneSqlGenerator.GetSelectAll(x => (x.Id == 456 && x.Number == "456") && x.Id == 123 && (x.Id == 4567 && x.Number == "4567"), null);
            Assert.Equal(
                sPrefix +
                "[DAB].[Phones].[Id] = @Id_p0 AND [DAB].[Phones].[Number] = @Number_p1 AND [DAB].[Phones].[Id] = @Id_p2 AND [DAB].[Phones].[Id] = @Id_p3 AND [DAB].[Phones].[Number] = @Number_p4",
                sqlQuery9.GetSql());
        }
        ...
        public static void SelectGroupConditionsNavigationPredicate()
        {
            ...
            Assert.Equal(
                sPrefix +
                "([Phones_PhoneId].[Number] = @PhoneNumber_p0 OR ([Users].[Name] = @Name_p1 AND [Phones_PhoneId].[IsActive] = @PhoneIsActive_p2)) AND [Users].[Deleted] != 1",
                sqlQuery1.GetSql());

            var ids = new List<int>();
            var sqlQuery2 = userSqlGenerator.GetSelectFirst(
                x => x.Phone.Number != "123" && (x.Name != "abc" || !x.Phone.IsActive || !ids.Contains(x.PhoneId) || !ids.Contains(x.Phone.Id)) &&
                     (x.Name == "abc" || x.Phone.IsActive), null, user => user.Phone);
            Assert.Equal(
                sPrefix +
                "([Phones_PhoneId].[Number] != @PhoneNumber_p0 AND ([Users].[Name] != @Name_p1 OR [Phones_PhoneId].[IsActive] = @PhoneIsActive_p2 OR [Users].[PhoneId] NOT IN @PhoneId_p3 OR [Phones_PhoneId].[Id] NOT IN @PhoneId_p4) AND ([Users].[Name] = @Name_p5 OR [Phones_PhoneId].[IsActive] = @PhoneIsActive_p6)) AND [Users].[Deleted] != 1",
                sqlQuery2.GetSql());
        }
